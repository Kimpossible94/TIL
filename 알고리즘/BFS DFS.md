...
이 경우에는 시작점을 2번 방문할 수도 있다.
2. 큐에 넣을 때 방문했다는 표시를 하는 대신 큐에서 빼낼 때 방문했다는 표시를 남긴다.  
같은 노드가 큐에 여러 번 들어가게 되어서 메모리 초과 또는 시간초과가 발생할 수 있다.  
3. 다음 노드 또는 원소를 검색할 때 주어진 범위를 벗어났는지에 대한 체크를 잘못했다.  
...
![alt text](./이미지/bfs_quiz_1.png)

좌표 (0,0)과 상하좌우로 이어진 모든 파란색 칸을 확인하시오.  
1. 우선 BFS 문제이므로 좌표를 담을 큐를 만든다.
2. 시작하는 칸을 방문했다고 표시를 하고 큐에 넣는다.
3. 큐에 넣은 칸을 꺼내고 상하좌우로 맞닿아있는 좌표중 방문하지 않고 파란색인 칸을 찾는다.  

이러한 과정을 반복해서 정답을 찾는다.  
모든 칸은 큐에 한 번씩만 들어가기 때문에 시간복잡도는 칸이 N개일 때 $O(N)$이 된다.  
만약 행이 R개 열이 C개이면 O(RC)가 된다.  
